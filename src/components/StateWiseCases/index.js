...
class StateWiseCases extends Component {
  ...
  getAllStatesData = async () => {
    ...
    const timelineUrl1 = `https://apis.ccbp.in/covid19-timelines-data`
    const timelineUrl2 = 'https://apis.ccbp.in/covid19-timelines-data/AP'
    const apiUrl = `https://apis.ccbp.in/covid19-state-wise-data/`
    const options = {
      method: 'GET',
    }
    const response = await fetch(apiUrl, options)
    const response1 = await fetch(timelineUrl1, options)
    const response2 = await fetch(timelineUrl2, options)
    if (response.ok) {
